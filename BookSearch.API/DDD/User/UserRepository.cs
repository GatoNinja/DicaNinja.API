...
public sealed class UserRepository : IUserRepository
{
    ...
    public async Task<UserModel?> DoLoginAsync(string username, string password)
    {
        var query = from user in Context.Users
                    where user.Username == username
                    select new UserModel(user.Id, user.Username, user.Email, user.Password);
    ...
    public async Task<UserModel?> InsertAsync(UserModel model)
    {
    ...
    public EnumNewUserCheck ValidateNewUser(UserModel userModel)
    {
